...
import filter.adapter.BaseBaseAdapter;
...
public class DoubleListView<LEFTD, RIGHTD> extends LinearLayout implements AdapterView.OnItemClickListener {
    ...
    private void init(Context context) {
        ...
        inflate(context, com.baiiu.filter.R.layout.merge_filter_list, this);

        lv_left = (ListView) findViewById(com.baiiu.filter.R.id.lv_left);
        lv_right = (ListView) findViewById(com.baiiu.filter.R.id.lv_right);
...
//            lv_left.performItemClick(mLeftAdapter.getView(checkedPositoin, null, null), checkedPositoin, mLeftAdapter.getItemId(checkedPositoin));//调用此方法相当于点击.第一次进来时会触发重复加载.
    ...
    public interface OnRightItemClickListener<LEFTD, RIGHTD> {
        void onRightItemClick(LEFTD item, RIGHTD childItem, int index);
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        if (parent == lv_left) {
            ...
            if (mOnRightItemClickListener != null) {
                mOnRightItemClickListener.onRightItemClick(mLeftAdapter.getItem(mLeftLastCheckedPosition), mRightAdapter.getItem(mRightLastChecked),index);
            }
        }
    }
