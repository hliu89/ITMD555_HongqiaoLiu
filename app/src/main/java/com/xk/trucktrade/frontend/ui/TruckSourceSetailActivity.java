...
public class TruckSourceSetailActivity extends BaseActivity {
    ...
    protected void fetchData() {
        ...
        tv_distance.setText(format + "kilometer");
        Log.e("ss", "fetchData" + format);
        if (trucksource.getUserBean().getAvatarurl().length() == 0) {
            Bitmap bitmap = avatarProducUtil.getBitmapByText(trucksource.getUserBean().getUsername().length() == 0 ? "no username" : trucksource.getUserBean().getUsername());
